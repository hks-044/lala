...
if (isset($_GET['id'])) {
    ...
    $query_gambar = "SELECT gambar FROM paket_internasional WHERE id = '$id'";
    ...
    $query_delete = "DELETE FROM paket_internasional WHERE id = '$id'";
    $hapus = mysqli_query($koneksi, $query_delete);

    if ($hapus) {
        echo "<script>
                alert('Data berhasil dihapus!');
                window.location = 'admin_internasional.php';
              </script>";
    } else {
        echo "<script>

...
if (isset($_POST['simpan'])) {
    $judul    = $_POST['judul'];
    $tanggal  = $_POST['tanggal']; // Pastikan name di form adalah 'tanggal'
    $durasi   = $_POST['durasi'];
    $harga    = $_POST['harga'];
    $lokasi   = $_POST['lokasi'];
    $deskripsi = $_POST['deskripsi']; // Pastikan name di form adalah 'deskripsi'
    ...
    if ($foto != "") {
        if (move_uploaded_file($tmp_foto, $path)) {
            // REVISI: Tabel paket_internasional & Kolom deskripsi_internasional
            $query = "INSERT INTO paket_internasional (judul, tanggal_keberangkatan, durasi, harga, lokasi, deskripsi_internasional, gambar) 
                      VALUES ('$judul', '$tanggal', '$durasi', '$harga', '$lokasi', '$deskripsi', '$nama_baru')";
            
            $result = mysqli_query($koneksi, $query);

            if ($result) {
                // REVISI: Redirect ke admin_internasional.php
                echo "<script>alert('Data berhasil disimpan!'); window.location='admin_internasional.php';</script>";
            } else {
...
<head>
    ...
    <title>Tambah Paket Internasional</title>
...
<body>
    ...
    <aside class="sidebar">
        ...
        <ul class="nav-menu">
            <li class="nav-item">
                <a href="admin_internasional.php" class="nav-link"><i class="fas fa-arrow-left"></i> KEMBALI</a>
            </li>
        </ul>
    </aside>
    ...
    <main class="main-content">
        <div class="page-header">
            <h1>Tambah Paket Internasional</h1>
            <div class="breadcrumb">Dashboard > Internasional > Baru</div>
        </div>
                ...
                <div class="input-column">
                    ...
                    <div class="form-grid">
                        <div class="form-group full-width">
                            <label>Nama Paket Internasional</label>
                            <input type="text" name="judul" placeholder="Contoh: Tour 3 Negara (Turki, Dubai, Cairo)" required>
                        </div>

                        <div class="form-group">
                            <label>Lokasi / Kota</label>
                            <input type="text" name="lokasi" placeholder="Istanbul, Cappadocia, Dubai" required>
                        </div>

                        <div class="form-group">
                            <label>Durasi Program</label>
                            <input type="text" name="durasi" placeholder="10 Hari" required>
                        </div>

                        <div class="form-group">
                            <label>Harga (Rp)</label>
                            <input type="number" name="harga" placeholder="18500000" required>
                        </div>

                        <div class="form-group">
                            <label>Estimasi Keberangkatan</label>
                            <input type="date" name="tanggal" required>
                        </div>

                        <div class="form-group full-width">
                            <label>Fasilitas & Itinerary</label>
                            <textarea name="deskripsi" placeholder="Tuliskan itinerary singkat dan fasilitas yang didapat..." required></textarea>
                            <p style="font-size:12px; color:#999; margin-top:5px;">Gunakan Enter untuk baris baru.</p>
                        </div>
                    </div>

                    <div class="btn-area">
                        <a href="admin_internasional.php" class="btn-cancel">Batal</a>
